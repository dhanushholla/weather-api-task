...
var obtainedData = async (url, options) => {
  try {
    ...
    windspeedcontent.innerText =
      "wind:" + response.current_observation.wind.speed + "km/hr";
    function addZero(i) {
      if (i < 10) {
        i = "0" + i;
      }
        ...
        document.getElementById("slider-wrapper").innerHTML += `
			<div class="forecast-days" id="forecast-days">
				...
				<div class="min-max-wrapper" id="min-max-wrapper">
					<div class="mintemp-wrapper" id="mintemp-wrapper">${
            " " + response.forecasts[i].low + "°C"
          }</div>
					<div class="maxtemp-wrapper" id="maxtemp-wrapper">${
            response.forecasts[i].high + "°C" + " | "
          }</div>
				</div>
			</div>`;
      }
    ...
    resobj.forEach((ele, i) => {
      if (i < 7) image1[i].src = getIcon(ele.text);
    ...
    function myFunction() {
	  ...
	  console.log("myfn call")
	}
    function alertFunc() {
		console.log("alertfn call")
      temperaturecontent.innerText = "INVALID INPUT!!";
      loaderwrap.innerHTML = "";
...
function getIcon(text) {
  switch (true) {
    case text.includes("Thunderstorms"):
    ...
    case text.includes("Fog") || text.includes("Haze"):
      return "images/fog.png";
    case text.includes("Clear"):
      return "images/sunny.png";
    case text.includes("Sun") || text.includes("Hot"):
